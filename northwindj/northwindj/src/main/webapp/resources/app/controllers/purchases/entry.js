...
app
		.controller(
				'purchasesEntryCtrl',
				function($scope, masterService, purchasesService,
						shippersService, $q, $pageUtils, $CommonUtils, $modal) {
					$scope.model = {};
					$scope.shippers = [];
					$scope.init = function() {
								...
								.then(
										function(data) {
											...
											if (data[1].data != null
												...
												$scope.model.shipper = $CommonUtils
														.convertNullToObject($scope.model.shipper);
												$scope.model.purchaseDate = new Date(
														$scope.model.purchaseDate);
												$scope.model.requireDate = new Date(
														$scope.model.requireDate);
												$scope.model.shippedDate = new Date(
														$scope.model.shippedDate);
												$scope.model.details = $CommonUtils
														.convertNullToArray($scope.model.details);
												for (var index = 0; index < $scope.model.details.length; index++) {
													var detail = $scope.model.details[index];
													detail.product = $CommonUtils
															.convertNullToObject(detail.product);
												}
											} else {
												$scope.model = {};
												$scope.model.id = 0;
					...
					$scope.findSupplier = function() {
						var modalInstance = $modal.open({
							templateUrl : url + 'suppliers/searchModal.html',
					...
					$scope.findEmployee = function() {
						var modalInstance = $modal.open({
							templateUrl : url + 'employees/searchModal.html',
					...
					$scope.findProduct = function(item) {
						var modalInstance = $modal.open({
							templateUrl : url + 'products/searchModal.html',
					...
					$scope.save = function() {
						purchasesService.save($scope.model).success(
								function(data) {
									window.location = url
											+ "purchases/entry.html?id="
											+ data.id;
								});
					};
					$scope.clear = function() {
						if ($scope.id == 0) {
							...
							window.location = url + "purchases/entry.html?id="
									+ $scope.id;
						}
					};
