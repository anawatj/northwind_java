 ...
 * 
 */
app.controller('customersEntryCtrl', function($scope, masterService,
		customersService, $pageUtils, $q, $CommonUtils) {
	...
	$scope.load = function() {
		...
		var promise = $q.all(
				[ masterService.getAllCountry(), masterService.getAllRegions(),
				...
				function(data) {
					...
					if (data[3].data != null && data[3].data != undefined
							&& data[3].data != "") {
						$scope.model = data[3].data;
						$scope.model.country = $CommonUtils
								.convertNullToObject($scope.model.country);
						if ($scope.model.country != {}) {
							$scope.countryChange();
						}
						$scope.model.city = $CommonUtils
								.convertNullToObject($scope.model.city);
						$scope.model.region = $CommonUtils
								.convertNullToObject($scope.model.region);
						$scope.model.demos = $CommonUtils
								.convertNullToArray($scope.model.demos);
	...
	$scope.countryChange = function() {
		masterService.getAllCity($scope.model.country.id).success(
				function(data) {
					$scope.cities = data.list;
				});
	};
	$scope.save = function() {
		customersService.save($scope.model).success(function(data) {
